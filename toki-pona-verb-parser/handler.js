    ...
    function getSimpleStructuredSentence(parseable) {
        ...
        tokens.forEach(function (token, index) {
            if (objectMarker.indexOf(token) > -1 &&
                ...
                return;
            } else if (verbMarker.indexOf(token) > -1 &&
                       index < tokens.length - 1) {
                sentence.push({part: 'verbMarker', sep: token, tokens: []});
                part = sentence[sentence.length -1];
                return;
...
module.exports = (context, callback) => {
  if (context == "") {
    throw("please give me text");
    return;
  }
    
  callback(undefined, sitelenParser.parse(context));
}
